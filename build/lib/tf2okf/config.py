...
DEFAULT = {
    'version': '2', 'framework': {'type': 'auto'},
    'terraform': {'root': '.', 'recursive_modules': True},
    'tfscaffold': {'components_dir':'components','modules_dir':'modules','variables_dir':'etc','include_components':['*'],'include_modules':True},
    ...
    'generation': {'resources':True,'modules':True,'dependencies':True,'inputs':True,'outputs':True,'providers':True},
    'knowledge': {'preserve_manual_files':True}, 'copilot': {'generate_instructions':True},
...
def validate(repo: Path, cfg: dict) -> dict:
    ...
    if framework not in {'auto','plain-terraform','tfscaffold','terragrunt'}: raise ValueError(f'Unsupported framework: {framework}')
    ...
    for key in ('components_dir','modules_dir','variables_dir'):
        ...
        if value: safe_repo_path(repo,value,label=f'tfscaffold.{key}')
    return cfg

def load(repo: Path) -> dict:
    path=repo/'.tf2okf.yml'
    if not path.exists(): return validate(repo,copy.deepcopy(DEFAULT))
    raw=yaml.safe_load(path.read_text(encoding='utf-8')) or {}
    if not isinstance(raw,dict): raise ValueError('.tf2okf.yml must contain a YAML mapping')
    mode=(raw.get('layout') or {}).get('mode')
    if mode and 'framework' not in raw: raw['framework']={'type':{'terraform':'plain-terraform'}.get(mode,mode)}
    return validate(repo,_merge(DEFAULT,raw))

def write_default(repo: Path, framework: str='auto') -> None:
    cfg=copy.deepcopy(DEFAULT); cfg['framework']['type']=framework
