...
from .model import TerraformModel, Resource, Module, Variable, Output, Provider
from .security import should_read, MAX_SUBPROCESS_OUTPUT

BLOCK_RE = re.compile(r'(?m)^\s*(resource|data|module|variable|output|provider)\s+"([^"]+)"(?:\s+"([^"]+)")?\s*\{')
REF_RE = re.compile(r'\b(?:data\.)?[A-Za-z_][\w-]*\.[A-Za-z_][\w-]*(?:\.[A-Za-z_][\w-]*)?|\bmodule\.[A-Za-z_][\w-]*|\bvar\.[A-Za-z_][\w-]*|\blocal\.[A-Za-z_][\w-]*')
...
def _balanced_block(text: str, open_pos: int) -> tuple[str, int]:
    ...
    while i < len(text):
        ...
        if c == '#': line_comment = True; i += 1; continue
...
def parse_terraform(root: Path, source_root: Path | None = None) -> TerraformModel:
    ...
    for path in sorted(root.rglob('*.tf')):
        if any(p in {'.terraform', '.git'} for p in path.parts) or not should_read(path, source_root): continue
        ...
        for m in BLOCK_RE.finditer(text):
            ...
            if kind in ('resource', 'data'):
                ...
                model.variables.append(Variable(first, attrs.get('type'), _clean_string(attrs.get('description')), default, attrs.get('sensitive','false').lower()=='true', rel))
            elif kind == 'output':
                model.outputs.append(Output(first, _clean_string(attrs.get('description')), attrs.get('value'), attrs.get('sensitive','false').lower()=='true', rel))
            elif kind == 'provider':
                if not any(p.name == first for p in model.providers): model.providers.append(Provider(first))
...
def terraform_docs_markdown(root: Path) -> str | None:
    ...
    if not exe:
        return None
    try:
        proc = subprocess.run([exe, 'markdown', 'table', str(root)], capture_output=True, text=True, check=False, timeout=30)
    except (subprocess.TimeoutExpired, OSError):
        return None
...
def enrich_from_terraform_docs(model: TerraformModel, doc: dict[str, Any] | None) -> TerraformModel:
    if not doc: return model
    inputs = doc.get('inputs') or []
    # terraform-docs JSON commonly emits lists of objects; support map form too.
    if isinstance(inputs, dict): inputs = [dict({'name': k}, **(v or {})) for k,v in inputs.items()]
    ...
    for item in inputs:
        ...
        if v:
            ...
            if item.get('type') is not None: v.type = str(item.get('type'))
            if 'default' in item: v.default = item['default']
            v.sensitive = bool(item.get('sensitive', v.sensitive))
    outputs = doc.get('outputs') or []
    if isinstance(outputs, dict): outputs = [dict({'name': k}, **(v or {})) for k,v in outputs.items()]
