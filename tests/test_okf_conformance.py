...
def test_generated_concepts_have_required_type(tmp_path: Path):
    (tmp_path/'main.tf').write_text('variable "env" { type = string }\nresource "null_resource" "x" {}\noutput "id" { value = null_resource.x.id }\n')
...
def test_manual_knowledge_survives_regeneration(tmp_path: Path):
    ...
    cfg=load(tmp_path); out=tmp_path/'.okf'; generate_bundle(parse_terraform(tmp_path),out,cfg)
    marker='\nHuman decision: do not destroy this.\n'; p=out/'knowledge/architecture.md'; p.write_text(p.read_text()+marker)
