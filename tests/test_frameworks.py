...
def test_explicit_framework_override(tmp_path: Path):
    ...
    (tmp_path / "bin").mkdir()
    (tmp_path / "bin" / "terraform.sh").write_text("")
    (tmp_path / "main.tf").write_text('resource "null_resource" "x" {}')
    cfg = load(tmp_path)
    cfg["framework"]["type"] = "plain-terraform"
    d = detect(tmp_path, cfg)
