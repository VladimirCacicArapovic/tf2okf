...
def _has_tf(repo: Path) -> bool:
    return any(
        p.is_file() for p in repo.rglob("*.tf") if ".terraform" not in p.parts and ".terragrunt-cache" not in p.parts
    )
...
def detect(repo: Path, cfg: dict | None = None) -> Detection:
    ...
    def add(name: str, points: int, reason: str):
        ...
        scores[name] = (score + points, reasons + [reason])

    components_dir = repo / "components"
    modules_dir = repo / "modules"
    etc_dir = repo / "etc"
    if (repo / "bin" / "terraform.sh").exists():
        add("tfscaffold", 45, "found bin/terraform.sh")
    if components_dir.is_dir():
        add("tfscaffold", 25, "found components/ directory")
    if etc_dir.is_dir() and any(etc_dir.glob("*.tfvars*")):
        add("tfscaffold", 15, "found tfvars under etc/")
    if components_dir.is_dir() and any(
        child.is_dir() and any(child.glob("*.tf")) for child in components_dir.iterdir()
    ):
    ...
    if etc_dir.is_dir() and (any(etc_dir.glob("env_*_*.tfvars*")) and any(etc_dir.glob("versions_*_*.tfvars*"))):
        add("tfscaffold", 10, "found tfscaffold env/versions tfvars naming under etc/")

    tg = [
        p
        for p in repo.rglob("terragrunt.hcl")
        if ".terragrunt-cache" not in p.parts and ".terragrunt-stack" not in p.parts
    ]
    stacks = [
        p
        for p in repo.rglob("terragrunt.stack.hcl")
        if ".terragrunt-cache" not in p.parts and ".terragrunt-stack" not in p.parts
    ]
    if tg:
        add("terragrunt", 70, f"found {len(tg)} terragrunt.hcl file(s)")
    if stacks:
        add("terragrunt", 15, f"found {len(stacks)} terragrunt.stack.hcl file(s)")
    if (repo / "root.hcl").exists():
        add("terragrunt", 10, "found root.hcl")

    if _has_tf(repo):
        add("plain-terraform", 30, "found Terraform (*.tf) source")
    ...
    if scores["terragrunt"][0] >= 70:
        winner = ("terragrunt", scores["terragrunt"])
    elif scores["tfscaffold"][0] >= 45:
        winner = ("tfscaffold", scores["tfscaffold"])
...
def discover_terragrunt(repo: Path, cfg: dict) -> TerragruntModel:
    ...
    configs = []
    for p in repo.rglob("terragrunt.hcl"):
        if any(part in ignore for part in p.relative_to(repo).parts):
            continue
        configs.append(p)
    for p in sorted(configs):
        text = p.read_text(encoding="utf-8", errors="replace")
        rel = p.relative_to(repo).as_posix()
        unit_rel = p.parent.relative_to(repo).as_posix()
        source = None
    ...
    for p in sorted(repo.rglob("terragrunt.stack.hcl")):
        if not any(part in ignore for part in p.relative_to(repo).parts):
            result.stack_files.append(p.relative_to(repo).as_posix())
    unit_configs = {u.config_file for u in result.units}
    for p in sorted(repo.rglob("*.hcl")):
        rel = p.relative_to(repo).as_posix()
        if (
            rel in unit_configs
            or rel in result.stack_files
            or any(part in ignore for part in p.relative_to(repo).parts)
        ):
            continue
...
def discover(repo: Path, cfg: dict):
    ...
    if d.framework == "tfscaffold":
        return d, discover_tfscaffold(repo, cfg)
    if d.framework == "terragrunt":
        return d, discover_terragrunt(repo, cfg)
