---
type: tfscaffold Terraform Unit
title: data
description: Terraform component `data` managed in a tfscaffold repository.
tags:
- terraform
- tfscaffold
- component
- data
generated:
  by: tf2okf/0.4.0
  at: '2026-09-17T19:58:26Z'
sources:
- id: source-1
  resource: ../../../../components/data/main.tf
  author: process:terraform
---


# data

This tfscaffold component summarizes the Terraform root under `components/data` and highlights its interface, dependencies, and generated references.

Kind: **tfscaffold component**

Source directory: `components/data`

- Resources/data sources: **0**
- Module calls: **2**
- Inputs: **8**
- Outputs: **3**

## Component description

<!-- tf2okf:manual-description-start -->
Add a detailed description of what this component is for, how it is used, and any operational caveats. Anything between the marker comments is preserved by `tf2okf generate`.
<!-- tf2okf:manual-description-end -->

## Knowledge

* [Inputs](inputs.md)
* [Outputs](outputs.md)
* [Providers](providers.md)
* [Dependencies](dependencies.md)

## Module calls

* [module.tags](module-calls/tags.md)
* [module.vpc](module-calls/vpc.md)
